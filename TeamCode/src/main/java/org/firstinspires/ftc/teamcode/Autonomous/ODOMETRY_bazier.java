...
public class ODOMETRY_bazier extends LinearOpMode {
    ...
    public void move2(double Speed, double kp_Turn, double kd_Turn, int y_positions, int x_positions, int Turn){
        ...
        while (myElapsedTime.seconds() < 30) {
            ...
            if (y_positions - y_odo >= 1000 || y_positions - y_odo <= -1000 || x_positions - x_odo >= 1000 || x_positions - x_odo <= -1000 || Math.abs(Turn - Turn_odo) >= 4) {
                ...
                MotorA.setPower(Math.min(Math.max((Math.sin(degree) * error_XY + (movement)), -Speed), Speed));
                MotorB.setPower(Math.min(Math.max((Math.cos(degree) * error_XY - (movement)), -Speed), Speed));
                MotorC.setPower(Math.min(Math.max((Math.cos(degree) * error_XY + (movement)), -Speed), Speed));
                MotorD.setPower(Math.min(Math.max((Math.sin(degree) * error_XY - (movement)), -Speed), Speed));
            } else if (y_positions - y_odo >= 300 || y_positions - y_odo <= -300 || x_positions - x_odo >= 300 || x_positions - x_odo <= -300 ) {
            ...
            else if (Math.abs(Turn - Turn_odo) >= 3) {
                MotorA.setPower(Math.min(Math.max(((movement)), -0.15), 0.15));
                MotorB.setPower(Math.min(Math.max((-(movement)), -0.15), 0.15));
                MotorC.setPower(Math.min(Math.max(((movement)), -0.15), 0.15));
                MotorD.setPower(Math.min(Math.max((-(movement)), -0.15), 0.15));
            }
            else {
                MotorA.setPower(0);
                MotorB.setPower(0);
    ...
    public void move_bazier(double Speed, double kp_Turn, double kd_Turn, int y_positions, int x_positions, int spot1_X, int spot1_Y , int spot2_X , int spot2_Y ,int Turn){
        double movement;
        ElapsedTime myElapsedTime;
        double pidXY = 0;
        double lastTime = 0;
